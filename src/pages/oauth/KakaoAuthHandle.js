...
const KakaoAuthHandle = (props) => {
    ...
    useEffect(() => {
        ...
        const kakaoLogin = async () => {
            try {
                const res = await axios.get(`http://localhost:8080/api/login/oauth2/code/kakao?code=${code}`);
                const authToken = res.data.authorization;
                console.log(res);
                ...
                if (tokenBody && tokenBody.status === 404 && tokenBody.message === "해당하는 정보의 사용자를 찾을 수 없습니다." && tokenBody.code === 2003) {
                    // 추가 정보 입력 페이지로 리다이렉트
